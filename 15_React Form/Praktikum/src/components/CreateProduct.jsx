...
export default function () {
  ...
  const [products, setProducts] = useState([]); // State untuk menampung data produk
  ...
  // Handle perubahan file input (gambar)
  const handleImageChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      // Set file image ke state
      setFormValues({
        ...formValues,
        image: file,
      });
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (validate()) {
      // Tambahkan produk baru ke dalam array products
      setProducts([
        ...products,
        {
          ...
          image: imagePreview, // Simpan URL gambar
        },
      ]);
